...
class MessageInput(BaseComponent):
    ...
    def setup_ui(self) -> None:
        ...
        self.message_type_combo.addItems(["Auto"])  # Auto-detect based on context
    ...
    def _send_message(self) -> None:
        ...
        if current_context == "common":
            message_type = "public"
            recipient = None
            logger.debug(f"MessageInput: Sending public message from context: {current_context}")
        else:
            message_type = "private"
            # Find the other participant in private context
            recipient = self._get_other_participant(current_context)
            if not recipient:
                logger.error(f"MessageInput: Cannot send private message: no recipient found for context {current_context}")
                return
            logger.debug(f"MessageInput: Sending private message to {recipient} from context: {current_context}")
        ...
        logger.debug(f"MessageInput: Sent {message_type} message: {content} (context: {current_context})")
    ...
    def _update_ui_for_context(self) -> None:
        ...
        if current_context == "common":
            ...
            if other_user:
                self.message_input.setPlaceholderText(f"Type your private message to {other_user}...")
            else:
