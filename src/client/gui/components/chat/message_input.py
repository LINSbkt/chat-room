...
"""
from src.utils.emoji_utils import replace_text_codes_with_emojis
...
class MessageInput(BaseComponent):
    ...
    def _send_message(self) -> None:
        ...
        if not content:
            return
        content = replace_text_codes_with_emojis(content)
        # Get current user
        ...
        self.publish_event(
            ...
            {"message": message_data, "context_id": current_context},
        )
    ...
    def _open_emoji_picker(self):
        ...
        current_user = self.get_state(StateKeys.CURRENT_USER)
        if not current_user:
            logger.error("Cannot open emoji picker: no current user")
            return
        emoji_picker = EmojiPicker(self, current_user)
    ...
    def _insert_emoji(self, emoji: str):
        ...
        new_text = (
            ...
            emoji + current_text[cursor_position:]
        )
