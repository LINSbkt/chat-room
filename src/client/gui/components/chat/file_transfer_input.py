...
class FileTransferInput(BaseComponent):
    ...
    # Signals
    # Emitted when file transfer is requested
    file_transfer_requested = Signal(dict)
    ...
    def _send_file(self) -> None:
        ...
        if not current_user:
            ...
            QMessageBox.warning(
                self, "Error", "You must be logged in to send files")
            return
        ...
        if file_size > max_size:
            QMessageBox.warning(
                self, "Error", f"File too large. Maximum size is {max_size // (1024*1024)}MB")
            return
        ...
        if current_context == "common":
            ...
            is_private = False
            logger.debug(
                f"FileTransferInput: Sending public file from context: {current_context}")
        else:
            # Private file transfer - send to specific user
            recipient = self._get_other_participant(current_context)
            if not recipient:
                logger.error(
                    f"FileTransferInput: Cannot send private file: no recipient found for context {current_context}")
                QMessageBox.warning(
                    self, "Error", "Cannot determine recipient for private file transfer")
                return
            is_private = True
            logger.debug(
                f"FileTransferInput: Sending private file to {recipient} from context: {current_context}")
        ...
        logger.debug(
            f"FileTransferInput: Requested {('private' if is_private else 'public')} file transfer: {os.path.basename(file_path)} (context: {current_context})")
        ...
        if current_context == "common":
            ...
            if other_user:
                ...
                self.file_button.setToolTip(
                    f"Send private file to {other_user}")
            else:
