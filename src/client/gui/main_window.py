...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.message_type_combo.currentTextChanged.connect(
        ...
        self.file_transfer_list.itemDoubleClicked.connect(
            self.on_file_transfer_item_clicked)
        ...
        self.downloads_list.itemDoubleClicked.connect(
    ...
    def set_chat_client(self, chat_client: ChatClient, username: str):
        ...
        chat_client.connection_status_changed.connect(
            self.on_connection_status_changed)

        # Connect file transfer signals
        chat_client.file_transfer_request.connect(
            self.on_file_transfer_request)
        chat_client.file_transfer_progress.connect(
            self.on_file_transfer_progress)
        chat_client.file_transfer_complete.connect(
            self.on_file_transfer_complete)
    ...
    def display_public_message(self, message):
        ...
        # Pull a theme color (accent/primary from Qt Material)
        palette = self.message_display.palette()
        system_color = palette.color(palette.Highlight)  # theme-safe

        # Create a text cursor at end
        cursor = self.message_display.textCursor()
        cursor.movePosition(QTextCursor.End)

        # Format this message only
        char_format = cursor.charFormat()
        char_format.setForeground(system_color)
    ...
    def display_private_message(self, message):
        ...
        # self.message_display.setTextColor(QColor(150, 0, 150))  # Purple for private messages
        self.message_display.append(formatted_message)
        self.message_display.setTextColor(QColor(255, 255, 255))  # Reset to white
        self.scroll_to_bottom()
    ...
    def send_message(self):
        """Send a message."""
        print(
            f"DEBUG: send_message called, client={self.chat_client}, connected={self.chat_client.connected if self.chat_client else None}")
        ...
        if message_type == "Public":
            ...
            if not recipient:
                QMessageBox.warning(
                    self, "Error", "Please select a recipient for private message")
                return
    ...
    def send_file(self):
        ...
        if message_type == "Public":
            ...
            if not recipient:
                QMessageBox.warning(
                    self, "Error", "Please select a recipient for private file transfer")
                return
        ...
        if file_path:
            ...
            if success:
                filename = file_path.split(
                    '/')[-1] if '/' in file_path else file_path.split('\\')[-1]
                self.display_system_message(
                    f"📤 Sending file '{filename}' to {recipient}...")
            else:
    ...
    def on_file_transfer_progress(self, transfer_id, current, total):
        ...
        for i in range(self.file_transfer_list.count()):
            ...
            if transfer_id in item.text():
                item.setText(
                    f"📁 {transfer_id}: {progress_percent:.1f}% ({current}/{total})")
                break
        else:
            # Add new item if not found
            self.file_transfer_list.addItem(
                f"📁 {transfer_id}: {progress_percent:.1f}% ({current}/{total})")
    ...
    def on_file_transfer_complete(self, transfer_id, success, file_path):
        ...
        if success and file_path:
            ...
            self.display_system_message(
                f"❌ File transfer failed: {transfer_id}")
    ...
    def on_file_list_received(self, file_list):
        ...
        if len(file_list) > 0:
            self.display_system_message(
                f"📋 Loaded {len(file_list)} accessible files")
        else:
    ...
    def open_downloads_folder(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Error Opening Folder",
                                 f"Could not open storages folder:\n{str(e)}")
